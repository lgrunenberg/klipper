...
class MCU_S42C_uart_bitbang:
    ...
    def build_config(self):
        ...
        self.mcu.add_config_cmd(
            "config_s42cuart oid=%d rx_pin=%s pull_up=%d tx_pin=%s bit_time=%d"
            % (self.oid, self.rx_pin, self.pullup, self.tx_pin, bit_ticks))
        self.s42cuart_send_cmd = self.mcu.lookup_query_command(
            "s42cuart_send oid=%c write=%*s read=%c",
            "s42cuart_response oid=%c read=%*s", oid=self.oid,
    ...
    def reg_write(self, instance_id, addr, reg, val, print_time=None):
        ...
        if self.analog_mux is not None:
            self.analog_mux.activate(instance_id)
        msg = self._encode_write(addr, reg, val)
...
def lookup_s42c_uart_bitbang(config, max_addr):
    ...
    if mcu_uart is None:
        mcu_uart = MCU_S42C_uart_bitbang(rx_pin_params, tx_pin_params,
                                        select_pins_desc)
...
class MCU_S42C_uart:
    ...
    def get_register(self, reg_name):
        ...
        with self.mutex:
            for retry in range(5):
                val = self.mcu_uart.reg_read(self.instance_id, self.addr, reg.addr, reg.len)
                if val is not None:
                    return val
    ...
    def set_register(self, reg_name, val, print_time=None):
        ...
        with self.mutex:
            for retry in range(5):
                 if self.mcu_uart.reg_write(self.instance_id, self.addr, 0x83, val,
                                        print_time):
                    return
        raise self.printer.command_error(
