...
ReadRegisters = {
    ...
    "POS_VALUE": {"addr": 0x36, "len": 6},
    "EN_STATUS": {"addr": 0x3A, "len": 3},
    "LOCK_STATE": {"addr": 0x3E, "len": 3},
}
...
WriteRegisters = {
    ...
    "SET_PROTECT": {"addr": 0x88, "len": 4},
    "SET_UNLOCK": {"addr": 0x3D, "len": 3},
...
class SERVO42C:
    def __init__(self, config):
        ...
        self.torque = config.getint(
            "torque", default=450, minval=0, maxval=MAX_TORQUE
        )

        self.kp = config.getint("kp", default=None)
        self.ki = config.getint("ki", default=None)
        self.kd = config.getint("kd", default=None)
    ...
    def _handle_connect(self):
        ...
        try:
            ...
            if self.kp is not None:
                self.set_kp(self.kp, None)
            if self.ki is not None:
                self.set_ki(self.ki, None)
            if self.kd is not None:
                self.set_kd(self.kd, None)
    ...
    def get_motor_lock(self):
        logging.info("getting lock state")
        lock_state = self.mcu_s42c.get_register("LOCK_STATE")
        logging.info("Lock state is %s", str(lock_state))
        if lock_state == bytearray(b"\x01"):
            return True
        return False

    def set_motor_unlock(self, print_time):
        self.mcu_s42c.set_register("SET_UNLOCK", 0, print_time)

    def set_enable(self, state, print_time):
        val = 0
        if state:
            if self.get_motor_lock():
                self.set_motor_unlock(print_time)
            val = 1
        try:
    ...
    def set_kp(self, kp, print_time):
        self.mcu_s42c.set_register("SET_KP", kp, print_time)

    def set_ki(self, ki, print_time):
        self.mcu_s42c.set_register("SET_KI", ki, print_time)

    def set_kd(self, kd, print_time):
        self.mcu_s42c.set_register("SET_KD", kd, print_time)
